...
least_squares = tf.norm((Yhat_tens - Y_tens)**2, ord=2)**2 / (featN - (b-a+1)) * 200
print "Done!\n"

#--------------------------- Define Optimizer --------------------------#
print "Building Optimizer ... ... ..."
train_op = tf.train.AdamOptimizer(learning_rate=0.08).minimize(least_squares)
print "Done!\n"
	...
	for i in range(1, epochs+1):
		...
		if i == 1 or i % 5 == 0:
			...
			for w in W_tens.eval()[0]:
				currStatus.append(w)
			maskedTable.append( currStatus )

			print "Epoch: " + str(maskedTable[-1][0]) + "\t\tLoss: " + str(maskedTable[-1][1])
			print W_tens.eval()
...
df = pd.DataFrame(maskedTable, columns = headers)
df.to_csv("results/Masked Losses_" + fileName[-11:-1] + ".csv")